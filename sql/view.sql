...
CREATE OR REPLACE VIEW v_filtre_cv AS WITH experience_totale AS (
        SELECT
            ec.id_candidat,
            SUM(
                EXTRACT(
...
CREATE OR REPLACE VIEW v_total_conges_paye_effectues AS
    SELECT
        e.id_employe,
        SUM(
            CASE
...
CREATE OR REPLACE VIEW v_informations_employe AS
    SELECT
        c.id_candidat,
        c.nom,
        c.prenom,
...
CREATE OR REPLACE VIEW v_csp_employe AS
    WITH competence_scores AS (
        SELECT 
            e.id_employe,
            e.id_candidat,
